...
export class Errors {
  ...
  showError(error) {
    ...
    newErrorElement.innerHTML = `<div class="error">${error}</div>`
    this.errorElement.appendChild(newErrorElement)
    let autoKill = setTimeout( () => {
      this.errorElement.removeChild(newErrorElement);
    }, 5000);
